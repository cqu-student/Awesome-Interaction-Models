...
| 2026-07  | DuplexChat: Constructing Speaker-Separated Full-Duplex Dialogue Speech at Scale for Spoken Dialogue Language Modeling  | [Paper](https://arxiv.org/abs/2607.04941) | [Code](https://github.com/sarulab-speech/DuplexChat) | 提出 DuplexChat-Pipe，从公共播客自动构建说话人分离的双通道全双工对话语料；发布约 41.5 万小时的 DuplexChat 数据集，覆盖 282,634 小时英语和 132,723 小时日语，并保留轮次切换、附和与重叠语音等自然对话动态|
| 2026-07  | Multimodal Voice Activity Projection for Turn-Taking in Social Robots with Voice-Activity-Related Pretrained Encoders  | [Paper](https://arxiv.org/abs/2607.07294) | [Code](https://github.com/acano15/MM-VAP) | 将 VAP 从纯音频扩展到同步音视频输入，利用与语音活动相关的预训练编码器和跨说话人注意力预测未来轮次动态，并面向社交机器人交互进行验证
| 2026-06 | Multi-Faceted Interactivity Alignment in Full-Duplex Speech Models | [Paper](https://arxiv.org/abs/2606.11167) | - | 通过强化学习直接优化停顿处理、轮次切换、附和反馈和用户打断四类交互行为，并通过语义质量奖励避免回复内容退化 |
| 2026-06 | Adaptive Turn-Taking for Real-time Multi-Party Voice Agents | [Paper](https://arxiv.org/abs/2606.13544) | - | 提出 ModeratorLM，在多方语音对话中根据助手承担的角色动态决定何时以及是否介入，通过流式 Speech LLM 联合建模轮次决策与回复生成，将 turn-taking 从固定触发规则扩展为角色条件下的主动参与策略 |
| 2026-05  | Thinking-while-speaking: A Controlled, Interleaved Reasoning Method for Real-Time Speech Generation  | [Paper](https://arxiv.org/abs/2605.20946) | -  | 提出 InterRS 受控交错推理方法，在实时语音生成过程中插入与表达进度对齐的推理步骤，结合交错监督微调与强化学习，在保持即时响应和自然流畅表达的同时提升数学与逻辑推理能力
| 2026-05  | DuplexSLA: A Full-Duplex Spoken Language Model with Synchronized Speech, Language, and Action  | [Paper](https://arxiv.org/abs/2605.20755) | [Code](https://github.com/hyzhang24/DuplexSLA)  | 提出全双工 Speech-Language-Action 模型，在统一的 160 ms 时间轴上联合建模用户语音、助手语音和结构化行动，使监听、说话、规划与工具调用能够同步进行，并发布 DuplexSLA-Bench 评测轮次控制和对话中工具调用能力
| 2026-05  | How Should LLMs Listen While Speaking? A Study of User-Stream Routing in Full-Duplex Spoken Dialogue  | [Paper](https://arxiv.org/abs/2605.10199) | [Demo](https://light1726.github.io/duplex-demo/)  | 系统研究全双工语音模型中的用户流路由问题，对比通道融合与交叉注意力两种方案；前者具有更强的语义整合能力，后者能更好地保护生成上下文，并在用户打断和语音重叠时保持稳健
| 2026-04  | Full-Duplex Interaction in Spoken Dialogue Systems: A Comprehensive Study from the ICASSP 2026 HumDial Challenge  | [Paper](https://arxiv.org/abs/2604.21406) | [Code](https://github.com/ASLP-lab/HumDial-FDBench)  | 发布基于真实人类双通道录音的 HumDial-FDBench，覆盖用户打断、重叠语音、附和、停顿、第三方语音与动态轮次协商，并从行为正确性、对话连续性和响应延迟等维度评测全双工语音系统
...
| 2025-07  | Triadic Multi-party Voice Activity Projection for Turn-taking in Spoken Dialogue Systems  | [Paper](https://arxiv.org/abs/2507.07518) | - | 首次将 VAP 从双人对话扩展到三人交互，通过联合预测多位参与者未来的语音活动实现多方轮次管理
...
|2025-05  | SALMONN-omni: A Standalone Speech LLM without Codec Injection for Full-duplex Conversation  | [Paper](https://arxiv.org/abs/2505.17060) | [Code](https://github.com/bytedance/SALMONN)  | 提出无需向 LLM 注入音频 codec token 的独立全双工语音模型，通过流式语音编码器、统一 LLM 与流式语音合成器实现同时监听和说话，并利用显式 thinking 状态预测轮次切换、附和、回声和上下文相关打断
...
| 2024-10 | OmniFlatten: An End-to-end GPT Model for Seamless Voice Conversation | [Paper](https://arxiv.org/abs/2410.17799) | [Page](https://omniflatten.github.io/) | 通过 progressive flattening 将模态对齐、半双工学习和全双工学习统一为标准 GPT 序列建模，无需修改语言模型主干架构 |
| 2024-10 | Yeah, Un, Oh: Continuous and Real-time Backchannel Prediction with Fine-tuning of Voice Activity Projection | [Paper](https://arxiv.org/abs/2410.15929) | [Code](https://github.com/ErikEkstedt/VoiceActivityProjection) | 在连续真实对话流中预测附和反馈的时机和类型，将 backchannel 与普通轮次切换和用户打断区分开来 |
| 2024-09 | Beyond Turn-Based Interfaces: Synchronous LLMs as Full-Duplex Dialogue Agents | [Paper](https://arxiv.org/abs/2409.15594) | [Page](https://syncllm.cs.washington.edu/) | 提出 SyncLLM，将现实时间信息注入自回归语言模型，使模型能够同步处理双方语流，并建模快速轮次切换、重叠语音和附和反馈 |
...
| 2024-03 | Multilingual Turn-taking Prediction Using Voice Activity Projection  | [Paper](https://arxiv.org/abs/2403.06487) | [Code](https://github.com/inokoj/VAP-Realtime) | 将 VAP 扩展到英语、中文和日语，表明单语模型难以直接跨语言迁移，而联合训练的多语言模型可以保持稳定的轮次预测性能 |
| 2024-01 | Real-time and Continuous Turn-taking Prediction Using Voice Activity Projection  | [Paper](https://arxiv.org/abs/2401.04868) | [Code](https://github.com/inokoj/VAP-Realtime) | 将 VAP 实现为实时连续轮次预测系统，直接从双通道对话音频预测未来语音活动，并可在 CPU 环境中实时运行 |
